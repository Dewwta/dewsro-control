...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        case static_cast<HRESULT>(D3DAPPERR_NODIRECT3D):
        ...
        case static_cast<HRESULT>(D3DAPPERR_NOCOMPATIBLEDEVICES):
            _tcscpy( strMsg, _T("Could not find any compatible Direct3D\n")
                             _T("devices.") );
            break;

        case static_cast<HRESULT>(D3DAPPERR_NOWINDOWABLEDEVICES):
        ...
        case static_cast<HRESULT>(D3DAPPERR_NODESKTOPHAL):
            _tcscpy( strMsg, _T("Your Direct3D hardware accelerator cannot\n")
        ...
        case static_cast<HRESULT>(D3DAPPERR_NOHALTHISMODE):
            _tcscpy( strMsg, _T("This sample requires functionality that is\n")
                             _T("not available on your Direct3D hardware\n")
        ...
        case static_cast<HRESULT>(D3DAPPERR_MEDIANOTFOUND):
        //case HRESULT_FROM_WIN32( ERROR_FILE_NOT_FOUND ):
            _tcscpy( strMsg, _T("Could not load required media." ) );
            break;

        case static_cast<HRESULT>(D3DAPPERR_RESETFAILED):
            _tcscpy( strMsg, _T("Could not reset the Direct3D device." ) );
            break;

        case static_cast<HRESULT>(D3DAPPERR_NONZEROREFCOUNT):
            _tcscpy( strMsg, _T("A D3D object has a non-zero reference\n")
                             _T("count (meaning things were not properly\n")
                             _T("cleaned up).") );
            break;

        case static_cast<HRESULT>(D3DAPPERR_NULLREFDEVICE):
