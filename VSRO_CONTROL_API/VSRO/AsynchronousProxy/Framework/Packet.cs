...
namespace VSRO_CONTROL_API.VSRO.AsynchronousProxy.Framework
{
    ...
    public class Packet
    {
        ...
        public void Skip(long count)
        {
            ...
            {
                if (!m_locked)
                    throw new Exception("Cannot Skip on unlocked Packet.");
        ...
        public long Position
        {
            get
            {
                ...
                {
                    if (!m_locked)
                        throw new Exception("Cannot read Position on unlocked Packet.");
        ...
        public long Length
        {
            get
            {
                ...
                {
                    if (!m_locked)
                        throw new Exception("Cannot read Length on unlocked Packet.");
        ...
        public byte[] PeekBytes(int count)
        {
            ...
            {
                if (!m_locked)
                    throw new Exception("Cannot PeekBytes on an unlocked Packet.");

                int available = Math.Min(count, RemainingRead());
                if (available == 0) return Array.Empty<byte>();

                long savedPos = m_reader.BaseStream.Position;
                byte[] buffer = m_reader.ReadBytes(available);
                m_reader.BaseStream.Seek(savedPos, SeekOrigin.Begin);
