...
namespace VSRO_CONTROL_API.VSRO.AsynchronousProxy.Network
{
    ...
    public class Client
    {
        ...
        public void BeginReceive()
        {
                ...
                Socket.BeginReceive(m_Buffer.Buffer, 0, m_Buffer.Buffer.Length, SocketFlags.None, (AsyncResult) =>
                {
                    try
                    {
                        ...
                        if (bytesRead > 0)
                        {
                            ...
                            {
                                Logger.Error(this, $"Security error, dropping connection: {secEx.Message}");
                                Close();
                                return;
                            }
                    ...
                    {
                        Logger.Error(this, $"Receive callback error: {ex.Message}");
                        try { Close(); } catch { }
                    }
            ...
            {
                Logger.Error(this, $"BeginReceive error: {ex.Message}");
            }
        }
        /// <summary>
