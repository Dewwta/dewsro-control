<script lang="ts">
    ...
    let botBusy = false;
    let botMessage = "";
    let botMsgType: MsgType = "info";
    let needConfirmBotStop = false;
    let confirmBotStopTimer: ReturnType<typeof setTimeout> | null = null;
    let botStatus: { isStarting: boolean; runningBots: string[] } | null = null;
    let botStatusInterval: ReturnType<typeof setInterval> | null = null;

    onMount(() => {
        fetchBotStatus();
        botStatusInterval = setInterval(fetchBotStatus, 5000);
    });
    ...
    async function fetchBotStatus() {
        try {
            botStatus = await serverApi.botStatus();
        } catch {
            /* silently ignore */
        }
    }
    ...
    async function handleStartBots() {
        ...
        botMessage = "";
        try {
            const res = await serverApi.startBots();
            botMessage = res.message;
            botMsgType = "success";
            botStatusInterval = setInterval(fetchBotStatus, 5000);
        } catch (e) {
            botMessage = e instanceof Error ? e.message : String(e);
            botMsgType = "error";
        } finally {
            botBusy = false;
        }
    }
    async function handleStopOne(windowTitle: string) {
        // Extract the name from "[botPartyOng] SBotP v1.0.38" → "botPartyOng"
        const match = windowTitle.match(/^\[(.+?)\]/);
        const name = match ? match[1] : windowTitle;
        botBusy = true;
        botMessage = "";
        try {
            const res = await serverApi.stopBot(name);
            botMessage = res.message;
            botMsgType = "success";
    ...
    async function handleRestartOne(windowTitle: string) {
        const match = windowTitle.match(/^\[(.+?)\]/);
        const name = match ? match[1] : windowTitle;
        botBusy = true;
        botMessage = "";
        try {
            const res = await serverApi.restartBot(name);
            botMessage = res.message;
            botMsgType = "success";
        } catch (e) {
            botMessage = e instanceof Error ? e.message : String(e);
    ...
    async function handleStopBots() {
        if (!needConfirmBotStop) {
            needConfirmBotStop = true;
            botMessage = "Click Stop again to confirm stopping all bots.";
            botMsgType = "info";
            if (confirmBotStopTimer) clearTimeout(confirmBotStopTimer);
            confirmBotStopTimer = setTimeout(() => {
                needConfirmBotStop = false;
                if (
                    botMessage ===
                    "Click Stop again to confirm stopping all bots."
                )
                    botMessage = "";
            }, 5000);
            return;
        ...
        botMessage = "";
        try {
            const res = await serverApi.stopBots();
            botMessage = res.message;
            botMsgType = "success";
            botStatus = null;
            if (botStatusInterval) clearInterval(botStatusInterval);
    ...
    async function handleStart() {
        if (!needConfirmStart) {
            ...
            confirmStartTimer = setTimeout(() => {
                ...
                if (message === "Click Start again to confirm server startup.")
                    message = "";
            }, 5000);
            return;
        }
    ...
    async function handleRestartShardGame() {
        if (!needConfirmShardGame) {
            ...
            message =
                "Click again to confirm Shard Manager + Game Server restart.";
            msgType = "info";
    ...
    $: isProxyRunning =
        status?.moduleStatuses?.find((m) => m.name === "Integrated Proxy")
            ?.isRunning ?? false;
    ...
    async function handleProxyStop() {
        if (!needConfirmProxyStop) {
            ...
            confirmProxyStopTimer = setTimeout(() => {
                ...
                if (
                    proxyMessage ===
                    "Click Stop again to confirm stopping the proxy."
                )
                    proxyMessage = "";
            }, 5000);
            return;
    ...
    async function handleShutdown() {
        if (!needConfirm) {
            ...
            confirmTimer = setTimeout(() => {
                needConfirm = false;
                if (message === "Click Shutdown again to confirm.")
                    message = "";
            }, 5000);
            return;
        }
...
<PageHeader
    ...
    subtitle="Start and stop the VSRO module stack"
>
...
<div class="page">
    ...
    <div class="ctrl-card">
        ...
        <p class="ctrl-card__desc">
            Starting the server launches all modules in sequence and automates
            the SMC node activation. The process takes approximately 2 minutes —
            do not interact with the server machine during startup. Shutdown
            kills all tracked processes immediately.
        </p>
    ...
    {#if isRunning}
        <div class="ctrl-card">
            ...
            <p class="ctrl-card__desc">
                ...
                <strong>Gateway</strong> restarts instantly — use after
                notice/launcher changes.
                <strong>Shard + Game</strong> kills both, relaunches them, waits
                ~35s for full init, then re-activates all nodes via SMC automatically.
            </p>
    ...
    <div class="ctrl-card">
        ...
        <p class="ctrl-card__desc">
            Controls the built-in gateway, download, and agent proxies. Use <strong
                >Start</strong
            >
            to bring the proxy up independently of the full server stack,
            <strong>Stop</strong> to tear it down, or <strong>Restart</strong> to
            cycle it without touching any other modules.
        </p>

        <div class="proxy-status">
            <span
                class="module-row__dot"
                class:dot--on={isProxyRunning}
                class:dot--off={!isProxyRunning}
            ></span>
            <span class="proxy-status__label"
                >{isProxyRunning ? "Proxy Running" : "Proxy Stopped"}</span
            >
        </div>
        ...
        {#if proxyMessage}
            <div class="msg msg--{proxyMsgType}" style="margin-top:0.6rem">
                {proxyMessage}
            </div>
        {/if}
    </div>
    ...
    <!-- Bot control card -->
    <div class="ctrl-card">
        <div class="ctrl-card__title">Bot Manager</div>
        <p class="ctrl-card__desc">
            Launches all configured SBot instances sequentially. Each bot logs
            in automatically and begins training. Stopping kills all bot
            processes immediately.
        </p>

        {#if botStatus?.runningBots?.length}
            <div class="bot-list">
                {#each botStatus.runningBots as bot}
                    <div class="bot-row">
                        <span class="dot--on"></span>
                        <span class="bot-row__name">{bot}</span>
                        <button
                            class="bot-row__kill"
                            title="Force stop {bot}"
                            disabled={botBusy}
                            on:click={() => handleStopOne(bot)}>✕</button
                        >
                        <button
                            class="bot-row__restart"
                            title="Restart {bot}"
                            disabled={botBusy}
                            on:click={() => handleRestartOne(bot)}>↺</button
                        >
                    </div>
                {/each}
            </div>
        ...
        {#if botStatus?.isStarting}
            <div class="stage-bar" style="margin-bottom: 0.75rem">
                <span class="stage-bar__label">Status</span>
                <span class="stage-bar__value">Starting bots...</span>
                <span class="stage-bar__spinner"></span>
            </div>
        {/if}
        ...
        <div class="ctrl-card__actions">
            <SrButton
                ...
                disabled={botBusy ||
                    ...
                    (botStatus?.runningBots?.length ?? 0) > 0}
                loading={botBusy}
                on:click={handleStartBots}
            >
                ▶ Start Bots
            </SrButton>
            ...
            <SrButton
                ...
                disabled={botBusy ||
                    ...
                    (botStatus?.runningBots?.length ?? 0) === 0}
                loading={botBusy}
                on:click={handleStopBots}
            >
                {needConfirmBotStop ? "⚠ Confirm Stop" : "■ Stop Bots"}
            </SrButton>
        </div>

        {#if botMessage}
            <div class="msg msg--{botMsgType}" style="margin-top:0.6rem">
                {botMessage}
            </div>
        {/if}
    </div>
        ...
        <div class="notice-form">
            <textarea
                ...
                on:keydown={(e) =>
                    e.key === "Enter" &&
                    !e.shiftKey &&
                    (e.preventDefault(), handleSendNotice())}
            ></textarea>
            <div class="notice-form__footer">
                {#if noticeMessage}
                    <span class="notice-msg notice-msg--{noticeMsgType}"
                        >{noticeMessage}</span
                    >
                {:else}
                    <span></span>
                ...
                {#each status.moduleStatuses as mod (mod.name)}
                    ...
                    >
                        ...
                        {#if mod.isRunning && mod.processId}
                            <span class="module-row__pid"
                                >PID {mod.processId}</span
                            >
                        {:else}
...
<style>
    ...
    .module-row--on .module-row__dot {
        background: var(--green-bright);
        box-shadow: 0 0 4px var(--green-bright);
    }
    .module-row--off .module-row__dot {
    ...
    .module-row--on .module-row__name {
        color: var(--text-base);
    }
